...
#
#
# # test_that('cv kfold_stratified (cat Y)', { Y=cbind(factor(c(rep('A',6),
# # rep('B', 6)))) cv_idc=.kFold(3, Y) cv_test=lapply(cv_idc, function(idc, k=3,
# # y_le=12){ which(!seq_len(y_le) %in% idc) }) cv_test=unique(unlist(cv_test))
# # expect_equal(length(cv_test), 12) expect_true(all(cv_test %in% 1:12)) })
#
#
#

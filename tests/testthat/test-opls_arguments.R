test_that("opls cv-method kfold", {
    ...
    # m <- ncol(iris) - 1
    #
    # smod <- opls(X = iris[, 1:4], Y = iris[, 5], cv = list(method = "k-fold", k = 7,
    ...
    smod <- opls(X, Y, cv = list(method = "k-fold", k = 7,split = 2/3), plotting = FALSE)
...
#     m <- ncol(iris) - 1
#
#     smod <- opls(X = iris[, 1:4], Y = iris[, 5], cv = list(method = "k-fold", k = n,
#         split = 2/3), plotting = FALSE)
#
#     expect_equal(class(smod)[1], "OPLS_metabom8")
#     expect_equal(nrow(smod@t_pred), n)
...
#     m <- ncol(iris) - 1
#
#     smod <- opls(X = iris[, 1:4], Y = iris[, 5], cv = list(method = "k-fold_stratified",
#         k = 7, split = 2/3), plotting = FALSE)
#
#     expect_equal(class(smod)[1], "OPLS_metabom8")
#     expect_equal(nrow(smod@t_pred), n)
#     expect_equal(nrow(smod@t_pred_cv), n)
...
#     k_high <- nrow(iris) - 10
#     smod <- opls(X = iris[, 1:4], Y = iris[, 5], cv = list(method = "k-fold_stratified",
#         k = k_high, split = 2/3), plotting = FALSE)
...
#     k_low <- 0
#     smod <- opls(X = iris[, 1:4], Y = iris[, 5], cv = list(method = "k-fold_stratified",
#         k = k_low, split = 2/3), plotting = FALSE)
...
#
#     k_high <- n + 300
#     smod <- opls(X = iris[, 1:4], Y = iris[, 5], cv = list(method = "k-fold", k = k_high,
#         split = 2/3), plotting = FALSE)
#     expect_true(smod@Parameters$cv$k < (k_high))
#
#     k_low <- 0
...
#     m <- ncol(iris) - 1
#
#     expect_error(opls(X = iris[, 1:4], Y = iris[, 5], cv = list(method = "MC", k = 10,
#         split = -1.5), plotting = FALSE))
#     expect_error(opls(X = iris[, 1:4], Y = iris[, 5], cv = list(method = "MC", k = 10,
#         split = 1.5), plotting = FALSE))
# })
#
...
#     # m <- ncol(iris) - 1
#     #
#     # smod <- opls(X = iris[, 1:4], Y = iris[, 5], cv = list(method = "MC", k = 10,
#     #     split = 2/3), plotting = FALSE)
#     #
#     data(covid)
#     Y=an$type
#
#     smod <- opls(X, Y, cv = list(method = "MC_balanced", k = 10, split = 2/3), plotting = FALSE)
#
#
#     expect_equal(class(smod)[1], "OPLS_metabom8")
#     expect_equal(nrow(smod@t_pred), n)
...
#     smod <- opls(X, Y, cv = list(method = "MC_balanced", k = 10, split = 2/3), plotting = FALSE)
#
#     smod <- opls(X = iris[, 1:4], Y = iris[, 5], cv = list(method = "MC_balanced",
#         k = 10, split = 2/3), plotting = FALSE)
#
#     expect_equal(class(smod)[1], "OPLS_metabom8")
#     expect_equal(nrow(smod@t_pred), n)
#     expect_equal(nrow(smod@t_pred_cv), n)

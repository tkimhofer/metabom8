...
test_that(".checkXclassNas fails for non-numeric matrix", {

  X <- matrix(letters[1:6], nrow = 3)

  expect_error(
    .checkXclassNas(X),
    "Input X must be a numeric matrix"
  )
...
test_that(".checkYclassNas preserves row order", {

  Y <- factor(c("A","B","A","B"))
  result <- .checkYclassNas(Y)
  expect_equal(nrow(result[[1]]), length(Y))
...
test_that(".prepareY keeps class order deterministic", {
  y <- c("b","a","c","a")
  result <- .prepareY(y)
  expect_equal(colnames(result[[1]]), sort(unique(y)))
})
...
test_that("prepareInputs produces consistent modeling input", {
  ...
  res <- .prepareInputs(X, Y, center = TRUE, scale = "UV")

  expect_true(is.matrix(res$X))
  expect_true(is.matrix(res$Y))
  expect_equal(nrow(res$X), nrow(res$Y))

# Function performs STOCSY analysis (NMR) Input: driver peak (ppm value), ppm
...
stocsy <- function(X, ppm, driver, plotting = TRUE, title = NULL) {
    ...
    if (dr_le > 1) {
        if (dr_le != nrow(X)) {
            stop("Ext driver does not match to X.")
        }
    ...
    if (!"matrix" %in% is(X) & is.numeric(X)) {
        stop("X argument is not a numeric matrix")
    }
    if (!is.numeric(driver) || (!extD & length(driver) > 1 && !(driver[1] <= max(ppm) & 
        driver[1] >= min(ppm)))) {
        stop("STOCSY driver not numeric or outside ppm range")
    }
    ...
    cc <- apply(X, 2, function(x, ll = l) {
        cor(x, ll)
    })
    cv <- apply(X, 2, function(x, ll = l) {
        cov(x, ll)
    })
    
    stoc_mod <- new("stocsy1d_metabom8", version = "0.9", X = X, ppm = ppm, driver = driver, 
        r = cc, cov = cv)
    ...
    if (plotting) {
        ...
        g1 <- ggplot(df, aes_string(x = "ppm", y = "cv", colour = "abs(cc)")) + geom_line() + 
            scale_x_reverse(breaks = breaks_pretty()) + scale_colour_gradientn(colours = matlab.like2(10), 
            limits = c(0, 1), name = csc_lab) + labs(x = expression(delta ~ {
        }^1 * H ~ (ppm)), y = gsub("^r", "cov", csc_lab), title = title, caption = cap) + 
            theme_bw() + theme(axis.text = element_text(colour = "black"), panel.grid.minor.x = element_blank())
        if (!extD) {
...
plotStocsy <- function(stoc_mod, shift = c(0, 10), title = NULL) {
    if (!"stocsy1d_metabom8" %in% is(stoc_mod)) 
        stop("Provide a STOCSY object")
    ds <- data.frame(r = stoc_mod@r, cov = stoc_mod@cov, ppm = stoc_mod@ppm, stringsAsFactors = FALSE)
    if (!all(is.numeric(shift))) 
        stop("Check shift argument")
    ...
    if (min(shift) < min(ds$ppm)) {
        shift[which.min(shift)] <- min(ds$ppm)
    }
    if (max(shift) > max(ds$ppm)) {
        shift[which.max(shift)] <- max(ds$ppm)
    }
    ...
    g1 <- ggplot(ds, aes_string(x = "ppm", y = "cov", colour = "abs(r)")) + geom_line() + 
        scale_x_reverse(breaks = breaks_pretty()) + scale_colour_gradientn(colours = matlab.like2(10), 
        limits = c(0, 1), name = csc_lab) + labs(x = expression(delta ~ {
    }^1 * H ~ (ppm)), y = gsub("^r", "cov", csc_lab), title = title, caption = cap) + 
        theme_bw() + theme(axis.text = element_text(colour = "black")) + ggtitle(title)

...
.vip <- function(Tx, W, C) {
  ...
  if (!is.matrix(Tx))
    stop("T must be a numeric matrix (n x A).")

  if (!is.matrix(W))
    stop("W must be a numeric matrix (A x p).")

  if (!is.numeric(Tx) || !is.numeric(W))
    stop("T and W must be numeric.")
  ...
  if (is.matrix(C)) {
    ...
    if (!all(dim(C) %in% c(1L, A)) || length(C) != A)
      stop("C must be length A, or a 1 x A / A x 1 matrix.")
    C <- as.numeric(C)
  } else if (is.numeric(C)) {
    if (length(C) != A)
      stop("Length of C must equal number of components (A).")
  } else {
    stop("C must be numeric.")
  }

  if (ncol(Tx) != A)
    stop("Number of components in T must match rows of W.")

  C <- as.numeric(C)

  if (length(C) != A)
    stop("Length of C must equal number of components.")

  SSY <- colSums(Tx^2) * (C^2)
  if (all(SSY == 0))
    stop("All SSY values are zero; cannot compute VIP.")

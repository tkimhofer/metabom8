...
opls_perm <- function(smod, n=10, plot=T, mc=F){
  ...
  #   out=mclapply(seq(n), do_perm, Y, cv, type, nc_o, mc.cores=numCores)
  # }else{
...
#     dn=pnorm(seq(0,1, by=0.01), mean(x.val), sd(x.val))
    ...
    ylab=ifelse(any(grepl('aucs', dd$variable)), 'AUROC', expression(R^2~'/'~Q^2))
    ...
    g=ggplot(dd, aes_string(x='abs(r)', y='value', colour='variable'))+
      geom_point()+
      theme_bw()+
      labs(x='|r|', y=ylab, colour='Sample Set', caption = paste0('O-PLS-', type, ', ', n, ' Y-permutations.'))

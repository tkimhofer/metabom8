...
#' @description Filters Bruker 1D experiments based on acquisition metadata and
#'   optionally limits the number of returned entries. Character parameters are
#'   matched by value, while numeric parameters support equality, set inclusion,
#'   range filtering (list(range = c(min, max))), and generic operators
#'   (list(op = ">", value = x)).
#' @param pars Data frame. Parsed acquisition and processing parameters.
#' @param exp_type Named list. Filtering conditions for parameters. Elements may
#'   be character vectors, numeric values/vectors, or operator/range lists for
#'   numeric fields.
#' @param f_list List. File paths of spectra and associated metadata (e.g., f_fid, f_1r).
#' @param n_max Integer. Maximum number of experiments to retain.
#' @return A list containing filtered and ordered `f_list` and `pars`.
#' @keywords internal
...
.filterExp_files <- function(pars, exp_type, f_list, n_max) {
  ...
  pars <- as.data.frame(pars, stringsAsFactors = FALSE)
  ...
  fmat <- vapply(seq_along(idx), function(i) {
    ...
    if (is.numeric(col)) {
      if (is.list(cond)) {
        if (!is.null(cond$range)) {
          lo <- cond$range[1]; hi <- cond$range[2]
          return(!is.na(col) & col >= lo & col <= hi)
        }
        if (!is.null(cond$op) && !is.null(cond$value)) {
          op <- cond$op
          val <- cond$value
      ...
      if (is.numeric(cond)) {
        ...
        return(!is.na(col) & col %in% cond)
      }
      if (is.character(cond) && all(grepl("^[0-9.]+$", cond))) {
...
#'
#' Internal utility that converts a named list into a single
#' character string of the form \code{name=value}. Nested
#' sublists are represented recursively using braces.
#'
...
#'
#' @details
#' This function is primarily used for compact parameter logging
#' in provenance metadata and should not be called directly by users.
#'
#' @keywords internal
.list_to_string <- function(lst) {
  paste(
    vapply(names(lst), function(nm) {
      val <- lst[[nm]]
      if (is.list(val)) {
        sprintf("%s={%s}", nm, .list_to_string(val))
      } else {
        sprintf("%s=%s", nm, val)
      }
    }, character(1)),

...
cvanova <- function(smod) {
    ...
    out <- data.frame(SS = c(format(ss, scientific = T), NA), DF = c(df, NA), MS = c(format(ms, 
        scientific = T), NA), F_value = c(rep(NA, 3), F_val), p_value = c(rep(NA, 
        3), format(p_val, scientific = TRUE)), row.names = c("Total corrected", "Regression", 
        "Residual", "RESULT"))
